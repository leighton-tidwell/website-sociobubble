...
else
{
	...
	while(($fetch = mysql_fetch_array($query)) != NULL)
	{
		if($fetch['from'] == $User->UserInfo['id'])
		{
			...
			echo("<img class='convopic'  src='p.php?src=".$User->UserInfo['profile_picture']."&h=50&w=50'><a href='" .$User->UserInfo['username']. "'>".$User->UserInfo['first_name']." ".$User->UserInfo['last_name']."</a><br>");
		...
		else
		{
			...
			{
				echo("<div id='convoitem' class='convoitem'>");
			echo("<img class='convopic' src='p.php?src=".$FriendInfo['profile_picture']."&w=50&h=50'><a href='" .$FriendInfo['username']. "'>".$FriendInfo['first_name']." ".$FriendInfo['last_name']."</a><br>");
			echo("<div class='italic'>This message was removed.</div>");
			echo("<div id='convofoot'>");
