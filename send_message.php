...
mysql_query("INSERT INTO `social_mail` (`userid`,`message`,`from`,`time`,`ipaddress`) VALUES ('".mysql_real_escape_string($_POST['select'])."','".mysql_real_escape_string($_POST['message'])."','".$User->UserInfo['id']."','".time()."','".mysql_real_escape_string($_SERVER['REMOTE_ADDR'])."')") or die(mysql_error());
...
$User->AddNotification($_POST['select'],$User->UserInfo['id'],$_POST['message'], $messagee, 2);
header("Location: messages");
exit;
?>